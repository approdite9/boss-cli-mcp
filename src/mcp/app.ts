#!/usr/bin/env node
/**
 * boss-mcp 共享运行时：把 boss-cli 的业务能力（`src/toolset` 的 `impl*`）组装成 MCP 能力集，
 * 并提供 {@link buildServer} 供各传输入口按会话构建 `Server` 实例。
 *
 * 本文件**不绑定任何传输**，也不含 `process.exit`：
 * - stdio 入口见 `src/mcp/server.ts`
 * - StreamableHTTP 入口见 `src/mcp/http_server.ts`
 *
 * 设计约束（与 `src/cli` 的差异）：
 ...
 *   这条约束是进程级的，**不随 MCP 会话数量增加而放宽**（见 {@link buildServer} 注释）。
 * - stdout 可能被 JSON-RPC 独占（stdio 传输），任何日志必须走 stderr。
 */
...
export const { name: pkgName, version: pkgVersion } = resolvePackageMeta();
...
/**
 * 每个 MCP 会话构建一套独立的 `Server` 实例。
 *
 * 为什么必须是工厂而不是单例：SDK 的 `Server` 内部记录初始化状态，一个实例只能 `initialize` 一次，
 * 复用它处理第二次 initialize 会被拒绝（"Server already initialized"）。HTTP 传输下客户端随时重连，
 * 每次重连都是一次新的 initialize，因此只能每次建新实例。
 *
 * 注意边界：**只有协议状态（Server / Transport）按会话隔离**。
 * 串行队列 `serialize()`、`browserCallThrottle`、浏览器会话与 `session.lock` 都必须保持进程级单份——
 * 本机只有一只 Chrome，按会话拆分会让多个客户端并发操作同一浏览器，直接击穿「同一 Chrome 不能并发」这条核心约束。
 */
export function buildServer(): Server {
  ...
  srv.setRequestHandler(CallToolRequestSchema, async (request, extra): Promise<CallToolResult> => {
 ...
 * detach CDP 的超时上限。
 *
 * Chrome 页面卡住时 `session.detach()` 会等 Chrome 回确认，实测可以拖到数分钟；
 * 退出流程不能被它拖死，所以这里给硬上限。注意**绝不调用 `browser.close()`**，
 * 用户的浏览器窗口要保留（见 `browser_session.ts` 的 `detachBrowserSession`）。
 */
const DETACH_TIMEOUT_MS = 5_000;

/**
 * 释放进程级共享资源：注销多实例登记 + 断开 CDP（保留浏览器窗口）。
 *
 * 只做「共享层」的事，**不含 `process.exit`**——退出时机由各入口自己决定，
 * 因为 HTTP 入口下单个 MCP 会话结束并不意味着进程该退出。
 */
export async function releaseSharedResources(): Promise<void> {
  unregisterInstance();
  await Promise.race([
    detachBrowserSession().catch(() => {}),
    new Promise((resolve) => setTimeout(resolve, DETACH_TIMEOUT_MS)),
  ]);
}
...
/**
 * 启动横幅：环境变量加载报告、节流设置、profile 登录态、调试端口现状、多实例冲突。
 *
 * 调试端口探测放在启动时，是为了把「端口上占着一只陈旧无头 Chrome」提前暴露，
 * 而不是等第一次工具调用才报错；真正的强制拦截仍在每次调用的 execute 里。
 */
export async function logStartupBanner(transportLabel: string): Promise<void> {
  console.error(
    [
      `[boss-mcp] ${pkgName} ${pkgVersion} MCP server 已启动（${transportLabel}，有头浏览器），共 ${TOOL_COUNT} 个工具`,
